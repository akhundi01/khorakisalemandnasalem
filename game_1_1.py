...
class App:
    def __init__(self):
        global score, high_score, active_score, floor_x, display_height, display_width
        ...
        game_font = pygame.font.Font(
            'img/font/Flappy.TTF', 40)  # فونت نرم افزار
        ...
        # هر چند لحظه یک بار به صورت خودکار اجرا می شود
        pygame.time.set_timer(create_pipe, 3000)

        # متغیری که فایل صوتی در آن قرار دارد
        win_sound = pygame.mixer.Sound('img/sound/smb_stomp.wav')
        game_over_sound = pygame.mixer.Sound('img/sound/smb_mariodie.wav')

        # متغیری که بک گراند صفحه در آن قرار دارد
        ...
        # متغیری که به صورت اتفاقی عکسی را انتخاب می کند
        image = pygame.image.load(
            'img/img/{}.png'.format(random.choice(['nasalem1', 'nasalem2', "nasalem3", "nasalem4"])))

        def generate_pipe_rect():  # تابعی که مکان قرار گیری غذا های ناسالم را به صورت اتفاقی انتخاب می کند
            # عدد را بین رنج داده شد به صورت اتفاقی انتخاب می کند
            random_pipe = random.randrange(300, 600)
            pipe_rect_top = image.get_rect(
                midbottom=(2111-300, random_pipe-260))
            pipe_rect_bottom = image.get_rect(midtop=(2111, random_pipe))
        ...
        def display_pipes(pipes):
            for pipe in pipes:
                if pipe.bottom >= 1024:
                    # قرار گیری غذا های ناسالم در پنجره
        ...
        def check_collision(pipes):
            global active_score
            for pipe in pipes:
                # چک می کنه که آیا برخوردی بوده یا نه
                if bird_image_rect.colliderect(pipe):
                    ...
                    active_score = True
                    return False
                # و اگه پرنده از محدوده های داده شده خارج شود
                if bird_image_rect.top <= -220 or bird_image_rect.bottom >= 900:
        ...
        def bird_animation():
            ...
            new_bird_rect = new_bird.get_rect(
                center=(90, bird_image_rect.centery))
        ...
        def display_score(status):
            ...
            if status == 'active':
                ...
                text1 = game_font2.render("Level 1", False, 'pink')
            ...
            for event in pygame.event.get():  # رویداد های را می گیرد مثلا اگه ماوس حرکت کنه متوجه می شود
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:  # چک می کنه که اگر کاربر روی کلید اسپیس رو فشار داد
                        bird_movement = 0
                        bird_movement -= 8  # مختصات صفحه هشت تا به سمت بالا حرکت کنه
                    if event.key == pygame.K_r and game_status == False:  # این شرط ها ترو برگردونن
                        ...
                        image = pygame.image.load('img/img/{}.png'.format(random.choice(
                            ['nasalem1', 'nasalem2', "nasalem3", "nasalem4"])))  # انتخاب تصویر
                        background_image = pygame.image.load(bg_random('bg'))

                        game_status = True
                        pipe_list.clear()
                        bird_image_rect.center = (40, 100)
                        bird_movement = 0
                        score = 0
