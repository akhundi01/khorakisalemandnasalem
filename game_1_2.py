...
class App:
    def __init__(self):
        ...
        image = pygame.image.load('img/img/{}.png'.format(random.choice(["nasalem3", "nasalem4",]))) # متغیری که به صورت اتفاقی عکسی را انتخاب می کند
        ...
        def display_score(status):
            ...
            if status == 'active':
                ...
                text1 = game_font2.render("Level 2", False, 'pink')
            ...
            for event in pygame.event.get(): # رویداد های را می گیرد مثلا اگه ماوس حرکت کنه متوجه می شود
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:# چک می کنه که اگر کاربر روی کلید اسپیس رو فشار داد
                        bird_movement = 0 
                        bird_movement -= 12 # مختصات صفحه دوازده تا به سمت بالا حرکت کنه
                    if event.key == pygame.K_r and game_status == False: # این شرط ها ترو برگردونن
                        ...
                        image = pygame.image.load(
                            'img/img/{}.png'.format(random.choice(["nasalem3", "nasalem4"])))  # انتخاب تصویر
